...
class Player:
    ...
    def update_session_stat(self, test, test2, user_ans, answer_label, q_left_label, score_label, live_label, user_entry, next_question, game_type, game_window):
...
def game_menu(window_type, difficulty_type, game_type):
    ...
    def submit_answer(user_entry, test, test2, window_type, game_type):
        TUTORIAL = "Enter an input using numbers, decimals or negative signs in the correct position."
        user_ans = user_entry.get()
        check_answer = False

        if "+" in user_ans:
            messagebox.showerror("Input not accepted", f"Positive signs are not allowed. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            user_entry.delete(0, END)
            return
        ...
        try:
            ...
            test2.update_session_stat(test, test2, user_ans, answer_label, q_left_label, score_label, live_label, user_entry, next_question, game_type, game_window)
            
        except ValueError:
            if not user_ans:
                messagebox.showerror("Input not accepted", f"Please enter an answer. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            elif any(user_input.isalpha() for user_input in user_ans):
                messagebox.showerror("Input not accepted", f"Alphabets are not allowed. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            elif "." in user_ans:
                messagebox.showerror("Input not accepted", f"This is an invalid decimal format. Please enter a valid decimal number. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            elif "+" in user_ans or "-" in user_ans:
                messagebox.showerror("Input not accepted", f"This is an invalid sign input. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            elif " " in user_ans:
                messagebox.showerror("Input not accepted", f"Spaces are not allowed. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            elif any((not user_input.isdigit() and not user_input.isalpha() for user_input in user_ans)):
                messagebox.showerror("Input not accepted", f"Special characters are not allowed. {TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            else:
                messagebox.showerror("Input not accepted", f"{TUTORIAL}\n\nYour Input: {user_ans}", parent=window_type)
            user_entry.delete(0, END)        
    ...
    live_label = Label(game_canvas, text= "Combo: 0", bg = COLOUR_6, font=(CUSTOM_FONT, 20, "bold italic"), fg=COLOUR_1, width = 30, height = 1)
    live_label.pack()
    ...
    submit_btn = Button(game_canvas, bg = COLOUR_5, font=(CUSTOM_FONT, 20, "bold italic"), text = "submit", fg=COLOUR_1, width = 30, height = 1, command = lambda: submit_answer(user_entry, test, test2, game_window, game_type))
    submit_btn.pack()
...
def profile_menu(window_type): 
    ProfileWidget = namedtuple('ProfileWidget', ['bg_color', 'txt_color', 'title', 'info'])
    widget_data = []
    user_xp = 0
    ...
    def load_items(file_score, file_combo, file_xp, file_question_asked, file_correct, file_game_played, file_perfect_game, cal_accuracy, cal_wrong):
        ...
        widget_data.append(ProfileWidget(COLOUR_8, COLOUR_12, "Highest Score", file_score))
        widget_data.append(ProfileWidget(COLOUR_8, COLOUR_12, "Highest Combo", file_combo))
        widget_data.append(ProfileWidget(COLOUR_8, COLOUR_12, "Perfect Games", file_perfect_game))
        widget_data.append(ProfileWidget(COLOUR_9, COLOUR_13, "Questions Asked", file_question_asked))
        widget_data.append(ProfileWidget(COLOUR_9, COLOUR_13, "Questions Correct", file_correct))
        widget_data.append(ProfileWidget(COLOUR_9, COLOUR_13, "Questions Wrong", cal_wrong))
        widget_data.append(ProfileWidget(COLOUR_10, COLOUR_14, "Accuracy", cal_accuracy))
        widget_data.append(ProfileWidget(COLOUR_10, COLOUR_14, "Games Played", file_game_played))
        widget_data.append(ProfileWidget(COLOUR_10, COLOUR_14, "Total XP", file_xp))
...
#====================================================================================================================================================================
    ...
    profile_window.configure (bg="#fffdf5")
