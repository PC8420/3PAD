...
# ====================================================================================================================================================================
    def submit_answer(user_entry, question_object, game_session, window_type, game_type):
        ...
        try:
            if game_type == GameType.FRACTIONS and "/" in user_ans:
...
def verify_create_account(window_type, user_password, username):
    ...
    if " " in verify_password or " " in verify_username:
        messagebox.showerror("Input not accepted", f"Spaces are not allowed. {LOGIN_TUTORIAL}", parent=window_type)
        return False

    if any((not user_input.isdigit() and not user_input.isalpha() for user_input in verify_username)):
    ...
    if any((not user_input.isdigit() and not user_input.isalpha() for user_input in verify_password)):
