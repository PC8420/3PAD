...
class GameSession:
    ...
    def question_timer(self):
        self.timer.after(2000)

    def confirm_answer(self, user_ans, raw_ans, question_object):
        self.questions_info.append([question_object.question_text, question_object.question_text_2, question_object.answer, user_ans])
...
class SimpleQuestion(Question):
    ...
    def exponent_mode(self):
        ...
        if self.operation == 1:
            ...
            operation = "+"
            self.question_text = f"({self.num_1}){formatted_num_2} {operation} ({self.num_3}){formatted_num_4}." 
            ...
            self.question_text = f"{self.num_2}[√({square_1}) {operation} √({square_2})]" 
            self.question_text_2 = f"Calculate"
    ...
    def algebra_mode(self):
        ...
        if self.operation == 1:
            ...
            self.answer = -1*self.num_2
            self.question_text = f"{(self.num_1*0.25)}x{power_numbers[2]} + {((self.num_1 *0.25)* (self.num_2*2))}x + {((self.num_1*0.25)*(self.num_2**2))} = 0"
            self.question_text_2 = f"Solve for x"
...
def score_menu(window_type, session_xp, combo, q_correct, total_questions, game_type, difficulty_type, game_session):
    ...
    score_window.configure (bg="#aaaaaa")
    score_window.title("Maths Mania! | Game")

    questions_data = game_session.questions_info
    #====================================================================================================================================================================
    ...
    score_top_frame = ctk.CTkFrame(score_main_frame, fg_color="#590b93", corner_radius=0)
    score_top_frame.pack(fill="x") 

    score_middle_frame = ctk.CTkFrame(score_main_frame, fg_color="transparent")
    score_middle_frame.pack(fill="x", padx=5, pady=(25,0)) 

    score_bottom_frame = ctk.CTkFrame(score_main_frame, fg_color="transparent")
    score_bottom_frame.pack(padx=20, pady=(25,0))

    # score_q_info_frame = ctk.CTkFrame(score_main_frame, fg_color=COLOUR_11)
    # score_q_info_frame.pack(fill="x", padx=20, pady=(20,0))

    # score_questions_frame = ctk.CTkFrame(score_main_frame, fg_color="transparent")
    # score_questions_frame.pack(fill="x", padx=20, pady=(20,0))

    score_button_frame = ctk.CTkFrame(score_main_frame, fg_color="transparent")
    score_button_frame.pack(pady=(20,0))

    #====================================================================================================================================================================

    main_info_frame = Frame(score_top_frame, width=530, height=150, bg="#590b93")
    main_info_frame.pack(pady=15) 

    score_difficulty_type = Difficulty[difficulty_type]
    score_game_type = GameType[game_type]

    mode_label = ctk.CTkLabel(main_info_frame, font=(CUSTOM_FONT_2, 35), text = score_game_type.name.capitalize(), fg_color=COLOUR_21, text_color=COLOUR_3, height=50, width=140, corner_radius=20)
    mode_label.grid(row=0, column=0, padx=(0,30))

    mode_label = ctk.CTkLabel(main_info_frame, font=(CUSTOM_FONT_2, 35), text = score_difficulty_type.name.capitalize(), fg_color=difficulty_colours[score_difficulty_type].colour_1, text_color=difficulty_colours[score_difficulty_type].colour_2, height=50, width=140, corner_radius=20)
    ...
    score_message_label = ctk.CTkLabel(score_middle_frame, text= score_message, fg_color = COLOUR_11, font=(CUSTOM_FONT_2, 35), height = 50, corner_radius=20, text_color=COLOUR_3)
    score_message_label.pack(fill="x", padx=20)

    score_label = ctk.CTkLabel(score_bottom_frame, text= f"Score: {session_xp}", fg_color = COLOUR_17, font=(CUSTOM_FONT_2, 35), height=50, width=140, corner_radius=20, text_color=COLOUR_16)
    score_label.grid(row=0, column=0)

    xp_label = ctk.CTkLabel(score_bottom_frame, text= f"+{session_xp} XP", fg_color = COLOUR_17, font=(CUSTOM_FONT_2, 35), height=50, width=250, corner_radius=20, text_color=COLOUR_16)
    xp_label.grid(row=0, column=1, padx=20)

    highest_combo_label = ctk.CTkLabel(score_bottom_frame, text= f"Best Combo: {combo}", fg_color = COLOUR_17, font=(CUSTOM_FONT_2, 35), height=50, width=140, corner_radius=20, text_color=COLOUR_16)
    highest_combo_label.grid(row=0, column=2)

    correct_label = ctk.CTkLabel(score_bottom_frame, text= f"{q_correct}/{total_questions} Correct", fg_color = COLOUR_17, font=(CUSTOM_FONT_2, 35), width=250, height = 50, corner_radius=20, text_color=COLOUR_16)
    correct_label.grid(row=0, column=3, padx=20)

    # view_questions_frame = ctk.CTkScrollableFrame(score_questions_frame, fg_color=COLOUR_15, scrollbar_button_color=COLOUR_17, scrollbar_button_hover_color=COLOUR_18)
    # view_questions_frame.pack(fill=BOTH, expand=True, pady=(0,20))

    # for q, question in enumerate(questions_data):
    #     if q % 2 == 0:
    #         q_bg = COLOUR_10
    #         q_label_bg = COLOUR_8
    ...
    #     question_frame = ctk.CTkFrame(view_questions_frame, height=40, corner_radius=0, fg_color=q_bg)
    #     question_frame.pack(fill="x", padx=(8,0), pady=5)

    #     q_label = ctk.CTkLabel(question_frame, font=(CUSTOM_FONT_2, 25), text = f"{question[1]}: {question[0]}", height=40, width=100, corner_radius=20, fg_color=q_label_bg)
    #     q_label.grid(row=0, column=1, padx=(0,0)) 

    #     model_ans_label = ctk.CTkLabel(question_frame, font=(CUSTOM_FONT_2, 25), text = f"{question[2]}", height=40, width=270, corner_radius=20, fg_color=q_label_bg)
    #     model_ans_label.grid(row=0, column=2, padx=(10,208)) 

    #     your_ans_label = ctk.CTkLabel(question_frame, font=(CUSTOM_FONT_2, 25), text = f"{question[3]}", height=40, width=220, corner_radius=20, fg_color=q_label_bg)
    #     your_ans_label.grid(row=0, column=3, padx=(10)) 

    # ==============================
    
    # info_question_label = ctk.CTkLabel(score_q_info_frame, font=(CUSTOM_FONT_2, 25), text = f"Question", height=40, width=100, corner_radius=20, fg_color=COLOUR_11, text_color=COLOUR_3)
    # info_question_label.grid(row=0, column=0, padx=(15,330)) 

    # info_ans_label = ctk.CTkLabel(score_q_info_frame, font=(CUSTOM_FONT_2, 25), text = "Correct Answer", height=40, width=270, corner_radius=20, fg_color=COLOUR_11, text_color=COLOUR_3)
    # info_ans_label.grid(row=0, column=1, padx=(0,225)) 

    # info_your_ans_label = ctk.CTkLabel(score_q_info_frame, font=(CUSTOM_FONT_2, 25), text = "Your Answer", height=40, width=220, corner_radius=20, fg_color=COLOUR_11, text_color=COLOUR_3)
    # info_your_ans_label.grid(row=0, column=2, padx=(0,15)) 

    # ==============================

    new_game_btn = ctk.CTkButton(score_button_frame, fg_color = COLOUR_23, font=(CUSTOM_FONT_2, 35), text = "New game", hover_color= COLOUR_32, text_color = COLOUR_24, height=50, width=140, corner_radius=20, command = lambda: mode_menu(score_window))
    new_game_btn.grid(row=0, column=0, padx=(0,20))

    home_btn = ctk.CTkButton(score_button_frame, fg_color = COLOUR_17, font=(CUSTOM_FONT_2, 35), text = "Home", text_color = COLOUR_16, hover_color= COLOUR_18, height=50, width=140, corner_radius=20, command = lambda: [score_window.destroy(), main_menu()])
...
# ====================================================================================================================================================================
    ...
    def check_answer(user_ans, raw_ans):
        ...
        game_window.after(1000, lambda: [feedback_display(COLOUR_22, COLOUR_22, COLOUR_1, img), submit_btn.configure(state = "normal"), user_entry.configure(state = "normal"), user_entry.delete(0, END), next_question(game_type, game_window)])
        game_window.after(1000, lambda: q_progress_bar.configure(progress_color=COLOUR_9))
# ====================================================================================================================================================================
    ...
    time_label = ctk.CTkLabel(time_frame, font=(CUSTOM_FONT_2, 40), text = f"3", fg_color=COLOUR_21, text_color=COLOUR_3)
    time_label.pack()
    #====================================================================================================================================================================
    ...
    display_question_label = ctk.CTkLabel(game_bottom_frame, text= "Starting in 3", fg_color = COLOUR_22, font=(CUSTOM_FONT_2, 40), text_color=COLOUR_1, width=1240, height=135, corner_radius=20)
    ...
    user_entry = ctk.CTkEntry(game_entry_frame, fg_color = "#aaaaaa", font=(CUSTOM_FONT_2, 35), corner_radius=20, placeholder_text = "Submit your answer here", width=960, height=50, state="disabled")
    user_entry.grid(row=0, column = 1, padx=(20,0))

    submit_btn = ctk.CTkButton(game_submit_frame, fg_color = COLOUR_21, font=(CUSTOM_FONT_2, 35), text = "Submit", hover_color=COLOUR_10, text_color=COLOUR_3, corner_radius=20, width = 960, height = 50, state = "disabled", command = lambda: validate_answer_input(user_entry, game_type))
...
def leaderboard_menu(window_type):
    ...
    leaderboard_banner_frame = Frame(leaderboard_window, width=530, height=150, bg="#590b93")
    ...
    # Adds a banner at the top of the screen
    leaderboard_banner_label = ctk.CTkLabel(leaderboard_banner_frame, font = (CUSTOM_FONT_2, 45), text = "Leaderboard Menu", text_color="#f3e1ff", fg_color="#590b93")
    leaderboard_banner_label.pack(fill=("x"), expand=True, padx=435, pady=15)

    # Creates a back_button, and calls for quit_program to destroy this window, then it calls the main_menu function to open the main menu.
    back_button = ctk.CTkButton(leaderboard_banner_frame, font=(CUSTOM_FONT_2, 35), text = "Back", height=50, width=140, corner_radius=20, fg_color=COLOUR_17, text_color=COLOUR_16, hover_color=COLOUR_18, command=lambda: [quit_program(leaderboard_window), main_menu()])
    back_button.place(x=1100, y=18)
    ...
    def leaderboard_info():
        ...
        heading_rank_label = ctk.CTkLabel(leaderboard_frame, font=(CUSTOM_FONT_2, 25), text = f"Rank", height=40, width=100, corner_radius=20, fg_color=COLOUR_5)
        heading_rank_label.grid(row=0, column=1, padx=(15,0)) 

        heading_username_label = ctk.CTkLabel(leaderboard_frame, font=(CUSTOM_FONT_2, 25), text = "Player", height=40, width=270, corner_radius=20, fg_color=COLOUR_5)
        heading_username_label.grid(row=0, column=2, padx=(0,217)) 

        heading_mode_label = ctk.CTkLabel(leaderboard_frame, font=(CUSTOM_FONT_2, 25), text = "Mode", height=40, width=220, corner_radius=20, fg_color=COLOUR_5)
        heading_mode_label.grid(row=0, column=3, padx=(10)) 

        heading_difficulty_label = ctk.CTkLabel(leaderboard_frame, font=(CUSTOM_FONT_2, 25), text = "Difficulty", height=40, width=150, corner_radius=20, fg_color=COLOUR_5)
        heading_difficulty_label.grid(row=0, column=4, padx=(10)) 

        heading_score_label = ctk.CTkLabel(leaderboard_frame, font=(CUSTOM_FONT_2, 25), text = "Score", height=40, width=200, corner_radius=20, fg_color=COLOUR_5)
        ...
        for i in leaderboard:
            ...
            rank_label = ctk.CTkLabel(player_frame, font=(CUSTOM_FONT_2, 25), text = f"{player_ranking}{player_title}", height=40, width=100, corner_radius=20, fg_color=leader_label_bg)
            rank_label.grid(row=0, column=1, padx=(0,0)) 

            username_label = ctk.CTkLabel(player_frame, font=(CUSTOM_FONT_2, 25), text = i[0], height=40, width=270, corner_radius=20, fg_color=leader_label_bg)
            username_label.grid(row=0, column=2, padx=(10,208)) 

            mode_label = ctk.CTkLabel(player_frame, font=(CUSTOM_FONT_2, 25), text = i[2].capitalize(), height=40, width=220, corner_radius=20, fg_color=leader_label_bg)
            mode_label.grid(row=0, column=3, padx=(10)) 

            difficulty_label = ctk.CTkLabel(player_frame, font=(CUSTOM_FONT_2, 25), text = i[3].capitalize(), height=40, width=150, corner_radius=20, fg_color=difficulty_colours[leader_difficulty_type].colour_1, text_color=difficulty_colours[leader_difficulty_type].colour_2)
            difficulty_label.grid(row=0, column=4, padx=(10)) 

            score_label = ctk.CTkLabel(player_frame, font=(CUSTOM_FONT_2, 25), text = i[1], height=40, width=200, corner_radius=20, fg_color=score_label_bg)
            score_label.grid(row=0, column=5, padx=(10,0)) 

            player_ranking = player_ranking + 1

    if leaderboard == []:
        no_data_label = ctk.CTkLabel(leaderboard_frame, height=40, corner_radius=10, font=(CUSTOM_FONT_2, 25), fg_color=COLOUR_5, text="No player data was found. Play a game to activate the leaderboard!")
        no_data_label.pack()
