...
class MathsQuiz:
    def __init__(self, window_type, difficulty, game_type):
        window_type.destroy()
        self.difficulty = difficulty

        self.game_window = Toplevel(login_window)
        self.game_window.geometry("1280x640")
        self.game_window.configure (bg="#584353")
        self.game_window.title("Maths Mania! | Game")
        self.img = tk.PhotoImage(file = "images/main_bg.png")
        self.quit = tk.PhotoImage(file = "images/quit.png")
        
        self.game_canvas = Canvas(self.game_window, width = 1280, height = 640, bg="purple")
        self.game_canvas.pack(fill="both", expand=True)
        self.game_canvas.create_image(0,0, image=self.img, anchor="nw")
        self.game_canvas.image = self.img

        #=========================================================================================================================================

        self.question = Question(difficulty, game_type)

        self.login_title_label = Label(self.game_canvas, text= self.question.text, bg = COLOUR_5, font=(CUSTOM_FONT, 20, "bold italic"), fg=COLOUR_1, width = 30, height = 1)
        self.login_title_label.pack()
        
        self.username_entry = Entry(self.game_canvas, bg = "#aaaaaa", font=(CUSTOM_FONT, 20, "bold"), width = 45)
        self.username_entry.pack()

        self.submit_btn = Button(self.game_canvas, bg = COLOUR_5, font=(CUSTOM_FONT, 20, "bold italic"), text = "submit", fg=COLOUR_1, width = 30, height = 1)
        self.submit_btn.pack()

    
    def display_question(self):
        self.login_title_label.config(text=f"What is {self.question.num_1} + {self.question.num_2}")
...
def main_game(difficulty, window_type):
    window_type.destroy()
    #quiz = quizapp(game_window, difficulty)
...
#    hard_button = Button(mode_canvas, bg = COLOUR_2, font = (CUSTOM_FONT, 18, "bold"), text = "HARD", command = lambda: main_game("HARD", mode_window), width = 15, height = 1)    
    ...
    def activate_btn(game_type):
        ...
        easy_button = Button(mode_canvas, bg = COLOUR_4, font = (CUSTOM_FONT, 18, "bold"), text = "EASY", command = lambda: MathsQuiz(mode_window, "EASY", game_type), width = 15, height = 1)
