...
class SimpleQuestion(Question):
    ...
    def generate_num(self, game_type):
        self.num_1 = random.randint(-10, 10)
        self.num_2 = random.randint(-10, 10)
        ans = self.num_1 + self.num_2

# class Question1:
#     def __init__(self, difficulty, game_type):
#         self.difficulty = difficulty
#         self.game_type = game_type
#         self.generate_num(difficulty, game_type)

#     def generate_num(self, difficulty, game_type):
#         if self.difficulty == "EASY":
#             self.num_1 = random.randint(0, 10)
#             self.num_2 = random.randint(0, 10)
...
#             self.num_1 = random.randint(-999, 999)
#             self.num_2 = random.randint(-999, 999)
            
        #self.display(game_type)
    
    def display(self, game_type):
        if game_type == "ADDITION":
            self.ans = self.num_1 + self.num_2
            self.text = f"What is {self.num_1} + {self.num_2}?"
        ...
        if game_type == "MULTIPLICATION":
            self.ans = self.num_1 * self.num_2
            self.text = f"What is {self.num_1} x {self.num_2}?"

    def verify_input(self):
        print("yes")
        user_ans = self.ans_entry.get()    
...
def score_menu(window_type):
    ...
    login_title_label = Label(score_canvas, text= f"You have scored ______ {p1.username}.", bg = COLOUR_5, font=(CUSTOM_FONT, 20, "bold italic"), fg=COLOUR_1, width = 30, height = 1)
    login_title_label.pack()

    submit_btn = Button(score_canvas, bg = COLOUR_3, font=(CUSTOM_FONT, 20, "bold italic"), text = "New game", fg=COLOUR_1, width = 30, height = 1, command = lambda: mode_menu(score_window))
    submit_btn.pack()
...
def game_menu(window_type, difficulty):
    window_type.destroy()
    ...
    login_title_label = Label(game_canvas, text= "", bg = COLOUR_5, font=(CUSTOM_FONT, 20, "bold italic"), fg=COLOUR_1, width = 30, height = 1)
    login_title_label.pack()
    
    ans_entry = Entry(game_canvas, bg = "#aaaaaa", font=(CUSTOM_FONT, 20, "bold"), width = 45)
    ans_entry.pack()

    submit_btn = Button(game_canvas, bg = COLOUR_5, font=(CUSTOM_FONT, 20, "bold italic"), text = "submit", fg=COLOUR_1, width = 30, height = 1)
    submit_btn.pack()
...
#    hard_button = Button(mode_canvas, bg = COLOUR_2, font = (CUSTOM_FONT, 18, "bold"), text = "HARD", command = lambda: main_game("HARD", mode_window), width = 15, height = 1)    
    ...
    def activate_btn(game_type):
        ...
        easy_button = Button(mode_canvas, bg = COLOUR_4, font = (CUSTOM_FONT, 18, "bold"), text = "EASY", command = lambda: score_menu(mode_window), width = 15, height = 1)
        win_easy_button = mode_canvas.create_window((195, 555), anchor= "nw", window=easy_button)

        medium_button = Button(mode_canvas, bg = COLOUR_5, font = (CUSTOM_FONT, 18, "bold"), text = "MEDIUM", command = lambda: game_menu(mode_window, "MEDIUM", ), width = 15, height = 1)
        win_medium_button = mode_canvas.create_window((525, 555), anchor= "nw", window=medium_button)
        
        hard_button = Button(mode_canvas, bg = COLOUR_2, font = (CUSTOM_FONT, 18, "bold"), text = "HARD", command = lambda: game_menu(mode_window, "HARD", ), width = 15, height = 1)
        ...
        banner_label.config(text= f"GAMEMODE SELECTED: {game_type}")
