...
power_numbers  = {-1: "⁻¹", -2: "⁻²", -3: "⁻³", -4: "⁻⁴", -5: "⁻⁵", -6: "⁻⁶", -7: "⁻⁷", -8: "⁻⁸", -9: "⁻⁹", 0: "⁰", 1: "¹", 2: "²", 3: "³", 4: "⁴", 5: "⁵", 6: "⁶", 7: "⁷", 8: "⁸", 9: "⁹"}
...
class SimpleQuestion(Question):
    ...
    def basic_operation(self, game_type, mode_setting, symbol):
        self.generate_two_num(game_type, mode_setting)

        if game_type != GameType.DIVISION:
            self.answer = calculate_type[symbol](self.num_1,self.num_2)
        ...
        self.question_text = f"{display_num_1} {symbol} {display_num_2}"
    ...
    def multiplication_mode(self):
        self.basic_operation(GameType.MULTIPLICATION, multiplication_settings, "x")
    ...
    def fraction_mode(self):
        def basic_fraction(symbol):
            self.answer = calculate_type[symbol](fraction_1, fraction_2)
            self.question_text = f"{self.num_1}/{self.num_2} {symbol} {self.num_3}/{self.num_4}" 
            self.question_text_2 = f"Calculate as a fraction or integer"
        ...
        if self.operation == 1:
            ...
            self.question_text = f"{self.num_1*self.num_2}/{self.num_3*self.num_4}"
            self.question_text_2 = "Simplify fully with the correct mathematical notation"
...
# ====================================================================================================================================================================
    ...
    def cancel_timer():
        nonlocal timer_id, reset_timer_id
        if timer_id is not None:
            game_window.after_cancel(timer_id)
            timer_id = None
        if reset_timer_id is not None:
            game_window.after_cancel(reset_timer_id)
            reset_timer_id = None

    def timer():
        nonlocal time_left, timer_id, reset_timer_id
...
# ====================================================================================================================================================================
    ...
    def check_answer(user_ans, raw_ans):
        ...
        if game_session.question_asked == QUESTIONS_PER_GAME:
            ...
            stat_question_label.configure(text = f"{game_session.question_asked+1}/{QUESTIONS_PER_GAME}")
        ...
        def reset_ui():
            if game_window.winfo_exists():
                feedback_display(COLOUR_22, COLOUR_22, COLOUR_1, img)
                submit_btn.configure(state = "normal")
                user_entry.configure(state = "normal")
                user_entry.delete(0, END)
                q_progress_bar.configure(progress_color=COLOUR_9)
                next_question(game_type, game_window)
        ...
        reset_timer_id = game_window.after(1000, reset_ui)
# ====================================================================================================================================================================
    ...
    def window_close():
        cancel_timer()
        game_window.destroy()
    game_window.protocol("WM_DELETE_WINDOW", window_close)
    ...
    correct_icon = ctk.CTkImage(light_image=Image.open("images/stats_correct.png"), dark_image=Image.open("images/stats_correct.png"), size=(40, 40))
    correct_image = ctk.CTkLabel(correct_frame, image=correct_icon, text="")  # display image with a CTkLabel
    correct_image.place(x = 130, y = 27)
    correct_image.icon = correct_icon
    #====================================================================================================================================================================
    ...
    stat_question_label = ctk.CTkLabel(questions_frame, font=(CUSTOM_FONT, 40), text = f"{"1"}/{QUESTIONS_PER_GAME}", fg_color=COLOUR_22, text_color=COLOUR_3)
    ...
    # Create and stores images.
    question_icon = ctk.CTkImage(light_image=Image.open("images/question_mark.png"), dark_image=Image.open("images/question_mark.png"), size=(40, 40))
    question_image = ctk.CTkLabel(questions_frame, image=question_icon, text="")  # display image with a CTkLabel
    question_image.place(x = 130, y = 27)
    question_image.icon = question_icon
    ...
    # Create and stores images.
    combo_icon = ctk.CTkImage(light_image=Image.open("images/punch.png"), dark_image=Image.open("images/punch.png"), size=(40, 40))
    combo_image = ctk.CTkLabel(combo_frame, image=combo_icon, text="")  # display image with a CTkLabel
    combo_image.place(x = 108, y = 30)
    combo_image.icon = combo_icon
    ...
    # Create and stores images.
    score_icon = ctk.CTkImage(light_image=Image.open("images/perfect_game.png"), dark_image=Image.open("images/perfect_game.png"), size=(40, 40))
    score_image = ctk.CTkLabel(score_frame, image=score_icon, text="")  # display image with a CTkLabel
    score_image.place(x = 164, y = 25)
    score_image.icon = score_icon
    #====================================================================================================================================================================
    ...
    answer_label = ctk.CTkLabel(game_bottom_frame, text= "", fg_color = COLOUR_22, font=(CUSTOM_FONT, 30), text_color=COLOUR_1, width=1240, height=40, corner_radius=20)
    answer_label.grid(row=1,column=0, pady=(0,20))

    display_question_label = ctk.CTkLabel(game_bottom_frame, text= "", fg_color = COLOUR_22, font=(CUSTOM_FONT, 40), text_color=COLOUR_1, width=1240, height=135, corner_radius=20)
    ...
    user_entry = ctk.CTkEntry(game_entry_frame, fg_color = COLOUR_4, font=(CUSTOM_FONT, 35), corner_radius=20, placeholder_text = "Submit your answer here", width=960, height=50)
    user_entry.grid(row=0, column = 1, padx=(20,0))

    submit_btn = ctk.CTkButton(game_submit_frame, fg_color = COLOUR_21, font=(CUSTOM_FONT, 35), text = "Submit", hover_color=COLOUR_10, text_color=COLOUR_3, corner_radius=20, width = 960, height = 50, command = lambda: validate_answer_input(user_entry, game_type))
...
#====================================================================================================================================================================
    ...
    for i, item in enumerate(widget_data):
        ...
        # Create and stores images.
        stat_icon = ctk.CTkImage(light_image=Image.open(item.image), dark_image=Image.open(item.image), size=(70, 70))
        stat_image = ctk.CTkLabel(stat_frame, image=stat_icon, text="")  # display image with a CTkLabel
        stat_image.place(x = 315, y = 15)
        stat_image.icon = stat_icon  # Storing reference so program doesn't delete/forget the image.
